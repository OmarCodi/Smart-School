...
if($request == 1){
  ...
  $resultat = mysqli_query($con, "SELECT * FROM `conatct`");
...
// verifier id
$record = mysqli_query($con,"SELECT * FROM `conatct` WHERE `Id_contact`like '".$id."'");
        if(mysqli_num_rows($record) > 0){

                mysqli_query($con,"DELETE FROM `conatct` WHERE `Id_contact` like '".$id."'");
                echo 1;
                exit;
        }
      else
...
if($request==3){
  ...
  if( $nom!='' && $prenom != '' && $tel != '' && $email != '' && $entr != '' && $choix != '' && $objet != '' && $text != '' ){

      mysqli_query($con,"INSERT INTO `conatct` VALUES (NULL,'".$nom."','".$prenom."','".$email."','".$tel."','".$entr."','".$choix."','".$objet."','".$text."')");
